#!/usr/bin/env python3
"""
Build an ensemble of the direct Tier 3 ATE predictions in ../results/.

Members (all calibrated at 0.35, all 208 condition x outcome cells):

    Qwen3.8-27B + HippoRAG            \\  averaged together first
    Qwen3.8-27B + HippoRAG g=0.70     /
    GPT-4o, full text
    GPT-5.4, full text
    Claude-Sonnet-4.6, full text

The nesting is the point. The two Qwen runs are the same checkpoint under two
retrieval settings and correlate at r=0.888, far above any cross-model pair
(the best is 0.844). A flat mean over the five files would give Qwen3.8 40% of
the ensemble and each API model 20%, purely because we happened to run Qwen
twice. Collapsing the Qwen pair first gives every MODEL an equal 25%.

Mean, not median: with four members the median is the average of the middle
two, which discards the outer two and moves discontinuously when they reorder.
The failure mode calibration exists to correct is systematic over-prediction,
not outliers, so there is nothing for a median to protect against here.
...
weighting, since these files carry point estimates with no standard errors.

Averaging in original units is safe: within an outcome every model reports on
the same scale, so mean(ate)/scale_range == mean(pp). Calibration is a positive
constant, so ensembling calibrated values equals calibrating the ensembled raw
values; the order does not matter.

Pass --flat to write the unnested five-file mean instead, for comparison. It
correlates with the nested mean at r=0.995 (mean difference 0.051 pp, max
0.288), so it is a sensitivity check rather than a real alternative.
"""
